...
function handle_generic_install(package_name) {
    print(`${sys_ok('')}${YELLOW}[WAIT FOR PROCESSING]${RESET}`);
    try {
        const [, , stderr, exit_status] = GLib.spawn_command_line_sync(`sudo apt install -y ${package_name}`);
        if (exit_status !== 0) {
...
this.install_command = function(package_name) {
    if (!package_name) {
